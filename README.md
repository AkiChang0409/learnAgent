...
- 根据学习主题生成结构化笔记
- 导入 Markdown 文档并生成“学科 > 主题 > 主笔记 > 分笔记”的知识地图
...
在学科首页或笔记页点击“导入 MD”，可以选择本地 `.md` / `.markdown` / `.txt` 文件。Agent 会读取文档内容，整理成一套结构化知识点：

- 主笔记：学科名、主题、整体摘要、功能全景或知识概览
- 分笔记：按文档结构拆出的子主题
- 补充内容：案例、易错点、面试/复习问题

如果导入的是项目开发文档，生成结果会重点提炼项目功能、亮点、技术重点、实现难点、对应解决方案和可复用经验，适合快速了解一个项目的技术方案。

长 Markdown 会先按标题和长度分块提炼，再汇总成最终知识地图，避免只总结文档开头。模型不可用时会使用本地规则兜底，仍可生成基础结构。
