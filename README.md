...
- 根据主题、问题或粘贴材料生成一篇经过聚焦规划与质量评审的结构化笔记
- 通过 Agent Persona 在学习笔记、Job Description 分析和项目代码技术解析之间切换
...
“AI 生成笔记”用于缩小范围生成一篇高质量笔记，不会把输入扩成完整知识地图。它支持短主题、核心问题和长材料输入，正常流程为：

1. Focus Planner：选择一个最有学习价值的核心问题，明确 `scopeIn` 和 `scopeOut`。
2. Writer：只围绕核心问题生成 4–6 个职责不同的小节，解释原理/机制、因果、边界、误区和迁移。
3. Quality Critic：检查范围失焦、长段复制、关键词堆砌、结构重复和解释浅显。
4. 定向重写：只有质量不合格时追加一次整体重写；最终还会通过本地规则清理复制段落和异常长标题。

长材料中的事实、合理推断和通用拓展会分别处理，避免把岗位描述、需求文档或其他粘贴内容重新排版后直接当成笔记。

## Agent Persona

统一的生成与 Markdown 导入入口支持三种内置专业模式：

- 学习笔记整理：聚焦原理、因果、边界、案例与迁移应用。
- Job Description 分析：提取职责、交付物、能力链、显隐性要求与申请准备；没有候选人经历时不会生成虚假匹配度。
- 项目代码技术解析：消费 Codex Skill 生成的代码分析 Markdown，整理架构、数据流、代码证据、工程取舍、风险和面试表达。

Persona 与快速、深度、离线等执行策略相互独立。Persona 会随笔记保存，并继续约束当前页问答、阶段记忆和对话回写。旧笔记自动兼容为 `learning-notes@1`。新增 Persona 的开发约定见 `AGENT_PERSONA_AUTHORING.md`。
...
Agent 会区分三层内容：原文或证据支持的事实、明确标注为“可推断”的合理推演，以及明确放在“拓展思考”中的通用原理、替代方案和演进建议。质量检查会拒绝只有“是什么”的浅层总结，要求继续解释机制或原理、因果关系、协作/数据过程、适用边界和取舍。

如果导入的是项目开发文档，生成结果会重点提炼项目功能、亮点、技术重点、实现难点、对应解决方案和可复用经验；一般知识文档则重点建立概念关系、解释原理、适用条件、易混淆点和迁移应用。

导入模式窗口会提供官方项目文档方案。也可以从左下角“小贴士”复制或下载 `codebase-technical-analysis-writer` Skill，并复制配套 Codex 指令，让 Codex 先读取项目代码生成高质量 Markdown，再导入 LearnAgent。
